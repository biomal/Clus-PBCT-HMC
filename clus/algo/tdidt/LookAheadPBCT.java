		...
		for (int i = 0; i < attrs.length; i++) {
			...
			if (at instanceof NominalAttrType) node.getFindBestTest().findNominal((NominalAttrType)at, node.getData());
                ...
                if(!(node.hasParent() && node.getParent().m_TypeSplit == VERTICAL_SPLIT)){
                    for (int i = 0; i < attrs.length; i++) {
                        ClusAttrType at = attrs[i];
			if (at instanceof NominalAttrType) node.getVerticalFindBestTest().findNominal((NominalAttrType)at, node.getVerticalData());
			else node.getVerticalFindBestTest().findNumeric((NumericAttrType)at, node.getVerticalData());
                ...
                if(bestAttrVertical!=null){
                    ...
                    if (bestAttrVertical instanceof NominalAttrType) node.getVerticalFindBestTest().findNominal((NominalAttrType)bestAttrVertical, node.getVerticalData());
                    else node.getVerticalFindBestTest().findNumeric((NumericAttrType)bestAttrVertical, node.getVerticalData());
                }
...
//			start_time = System.currentTimeMillis();
			if(node.getTypeSplit()==HORIZONTAL_SPLIT){
                            ...
                            for (int j = 0; j < arity; j++) {
                                    ...
                                    newPBCTNode.m_FTest = node.m_FTest;
